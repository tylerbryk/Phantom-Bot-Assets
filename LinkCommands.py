...
coc_client = coc.login('phantomfamilybot@gmail.com',
                       'PhantomBot1234',
                       client=coc.Client,
                       key_names='Phantom Bot',
                       key_count=1)
...
class LinkCommands(commands.Cog):
    ...
    async def force_update(self, ctx):
        t0 = time.time()
        msg = await ctx.send(embed=discord.Embed(title='Updating Database...', description='ETA: 60 seconds', color=0xf1c40f))
    ...
    async def connect_db():
        db = pymysql.connect(
            ...
            user='u550782982_phantom',
            password='PhantomBot1234',
            database='u550782982_phantom')
        return db, db.cursor()
